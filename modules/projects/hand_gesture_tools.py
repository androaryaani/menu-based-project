...
def project_game_controller():
    ...
    st.write("Control games using hand gestures")

    # Initialize session state
    if 'game_window' not in st.session_state:
        st.session_state.game_window = "Hill Climb Racing"
    if 'game_status' not in st.session_state:
        st.session_state.game_status = "Stopped"
    if 'run_camera_game' not in st.session_state:
        st.session_state.run_camera_game = False

    def focus_game_window(title):
        """Focus on game window (simulated)"""
        st.session_state.game_window = title
        st.success(f"🎯 Focused on: {title}")
    ...
    with col1:
        ...
        # Game selection
        game_title = st.text_input("Game Window Title:", value="Hill Climb Racing", key="game_title")
        
        if st.button("🎯 Focus Game", key="focus_game_btn"):
            focus_game_window(game_title)
        
        # Camera control buttons
        col_cam1, col_cam2 = st.columns(2)
        with col_cam1:
            if st.button("📷 Start Camera", key="start_cam_game"):
                st.session_state.run_camera_game = True
        with col_cam2:
            if st.button("🛑 Stop Camera", key="stop_cam_game"):
                st.session_state.run_camera_game = False
        
        # Camera feed
        FRAME_WINDOW_GAME = st.image([], use_column_width=True)
        
        # Status display
        status_placeholder_game = st.empty()
    ...
    with col2:
        ...
        st.write(f"{status_color[st.session_state.game_status]} **Status:** {st.session_state.game_status}")
        st.write(f"🎯 **Focused Window:** {st.session_state.game_window}")
        
        # Game info
        st.subheader("🎯 Game Info")
        st.write("**Supported Games:**")
        st.write("• 🏎️ Hill Climb Racing (Default)")
        st.write("• 🎮 Minecraft")
        st.write("• 🏁 Racing Games")
        st.write("• 🎯 FPS Games")
        st.write("• 🧩 Puzzle Games")
        
        # Gesture guide
        st.subheader("📖 Gesture Guide")
        st.write("**Open Hand (4-5 fingers):** Start/Resume")
        st.write("**Closed Fist (0 fingers):** Pause")
        st.write("**1 Finger:** Resume")
        st.write("**2 Fingers:** Stop")
        st.write("**3 Fingers:** Next Level")
        st.write("**Thumbs Up:** Previous Level")

    # Camera processing for game controller
    if st.session_state.run_camera_game:
        # Mediapipe init
        mp_hands = mp.solutions.hands
        hands = mp_hands.Hands(max_num_hands=1)
        mp_drawing = mp.solutions.drawing_utils
        ...
        try:
            ...
            while st.session_state.run_camera_game:
                ret, frame = cap.read()
                if not ret:
                    st.warning("⚠️ Failed to grab frame")
                    break
                ...
                if results.multi_hand_landmarks:
                    for lm in results.multi_hand_landmarks:
                        ...
                        # Process game controls based on finger count
                        if fingers >= 4:  # Open hand
                            if st.session_state.game_status != "Running":
                                st.session_state.game_status = "Running"
                                status_placeholder_game.success("🎮 Game started!")
                        elif fingers == 0:  # Closed fist
                            if st.session_state.game_status != "Paused":
                                st.session_state.game_status = "Paused"
                                status_placeholder_game.warning("⏸️ Game paused")
                        elif fingers == 1:  # Resume
                            if st.session_state.game_status == "Paused":
                                st.session_state.game_status = "Running"
                                status_placeholder_game.success("▶️ Game resumed!")
                        elif fingers == 2:  # Stop
                            if st.session_state.game_status != "Stopped":
                                st.session_state.game_status = "Stopped"
                                status_placeholder_game.error("⏹️ Game stopped")
                        elif fingers == 3:  # Next level
                            status_placeholder_game.success("🚀 Next level!")
                        
                        # Display game status on frame
                        cv2.putText(frame, f"Game: {st.session_state.game_status}", (10, 100), 
                                   cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                        cv2.putText(frame, f"Window: {st.session_state.game_window}", (10, 140), 
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)

                FRAME_WINDOW_GAME.image(frame, channels="BGR")
                time.sleep(0.05)
...
def hand_gesture_projects_menu():
    ...
    tab1, tab2, tab3 = st.tabs([
        "🎵 Music Controller", "🧮 Calculator", "🎮 Game Controller"
    ])
